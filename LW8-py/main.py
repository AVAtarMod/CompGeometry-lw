...
def window():
   ...
   global button_bar_gen, button_make_curve, \
   ...
   with dpg.menu_bar(parent=w):
      ...
      button_make_curve = dpg.add_menu_item(
          label="Построить фрактал", enabled=False)
...
def set_callbacks():
   ...
   s_pool["btn_clip"] = EventHandlerPool(button_make_curve)
   # Dynamic point pool
   ...
   s_pool["settings_point_amount"] += TextHandlers.set_plot_number_amount(
       button_make_curve)
   dyn_pool["frame_point"] += PointHandlers.update_points(
       plot)

   s_pool["btn_point_gen"] += ButtonHandlers.generate_points(
       plot, dyn_pool["frame_point"])
   s_pool["btn_point_gen"] += ButtonHandlers.enable_items([button_make_curve])
 
   s_pool["btn_clip"] += ButtonHandlers.make_curve(plot)
...
def viewport():
   dpg.create_viewport(title='Bezier Curves GUI',
