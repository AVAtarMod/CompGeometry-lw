...
void task1_2()
{
   ...
   if (!in) {
      ...
      if (i == 1)
         std::cout << "Counterclockwise orientation" << std::endl;
      else if (i == -1)
         std::cout << "Clockwise orientation" << std::endl;
   }
   in.close();
}
...
namespace t1_3 {
   bool equal(double a,
              double b) // Are the points on the same side of the segment?
   {
      if (tools::isZero(a) || tools::isZero(b) || (tools::sign(a) == tools::sign(b)))
         return true;
      else
         return false;
   }
   ...
   bool isIntersection(std::vector<Point> points)
   {
      ...
      if (tools::isZero(p1) && tools::isZero(p2)) // Do the segments lie on the same line?
      {
         ...
         if (tools::isZero(dis1 - dis) || tools::isZero(dis2 - dis))
            return true;
         return false;
      } else if (equal(p1, p2) || equal(p3, p4))
         return true;
      return false;
   }
} // namespace t1_3
...
namespace t1_4 {
   ...
   bool isConvex(std::vector<Point> points)
   {
      ...
      int _sign = tools::sign(
        ((points[s - 1] - points[s - 2]) ^ (points[0] - points[s - 2]))[2]);
      int temp =
        tools::sign(((points[0] - points[s - 1]) ^ (points[1] - points[s - 1]))[2]);
      if (!equal(_sign, temp))
         return false;

      for (int i = 0; i < s - 2; i++) {
         temp =
           tools::sign(((points[i + 1] - points[i]) ^ (points[i + 2] - points[i]))[2]);
         if (!equal(_sign, temp))
            return false;
      }
      return true;
