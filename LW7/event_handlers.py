   ...
   def generate_points(plot_id: int | str, point_callback):
      ...
      def __update_func(sender: int | str, _0=None, _1=None):
         ...
         for i in range(plot_number_amount):
            ...
            p = get_unique_point(_round_func, _point_size, points=points)
            _plot_points[label] = PlotPoint(0, p)
            id = dpg.add_drag_point(default_value=(
                p['x'], p['y']), label=label, callback=point_callback,
                parent=plot_id, color=generate_color())
            _plot_points[label].id = id
